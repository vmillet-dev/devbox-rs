 ...
 *
 * `keys` is already split — `['Ctrl', 'K']` and not `'Ctrl+K'` — because each
 * one is rendered as its own `<kbd>`. A step that is not a key press (`Ctrl` +
 * click, a drag) is spelled out in the label instead: a `<kbd>Click</kbd>`
 * would read as a key to find on the keyboard.
 */
 ...
 * dialog reads them from `SettingsStore` so the sheet shows what is really
 * bound.
 */
